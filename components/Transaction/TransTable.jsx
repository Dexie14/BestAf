    ...
    return (
      <div>
        <table className=" w-full table-auto tabling">
          ...
          <tbody className="bg-white cursor-pointer " onClick={() => setIsOpen(true)}>
            {/* {data &&
                    data?.data?.map((item: any, index: number) => ( */}
            <tr className="bg-white border-b-4" style={{boxShadow:"0px 2px 2px 0px rgba(34, 34, 34, 0.10);"}}>
              <td className="pl-2">
              <input type="checkbox"/>
              </td>
      ...
      <ReactModal
        ...
        style={customStyles}
      >
        <TransDetails setModalIsOpen={setIsOpen} modalIsOpen={isOpen} />
      </ReactModal>
      </div>
    );
  };
